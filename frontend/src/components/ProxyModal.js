...
const ProxyModal=({user, setUser, setClick})=>{
    ...
    <div id="proxy_container">
      <div id="proxyModal">
        {content === "select" && (<ProxyTable user={user} row={row} setRow={setRow} setContent={setContent} setClick={setClick}/>)}
...
const ProxyTable =({user, row, setRow, setContent, setClick})=>{
    const {peers} = useContext(AppContext);
...
const Options = ({row, setRow, setClick, user, setUser, setContent}) =>{
    ...
    return(
        <div id="confirmation">
            <h3 style={{ color: mode === "dark" ? "black" : "black" }}>Are you sure you want to perform a proxy connection with a cost of {row.Price} OrcaCoins per MB?</h3>
            <div id="buttons">
